...
class DGM_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # ---- Update notification (banner only — full controls in Preferences) ----
        if updater._update_available:
            box = layout.box()
            row = box.row(align=True)
            row.label(text="Update {} available!".format(updater._latest_version_str), icon='INFO')
...
# ---------------------------------------------------------------------------
# Addon Preferences — shown in Edit > Preferences > Add-ons > DayZ Geometry Maker
# ---------------------------------------------------------------------------

class DGM_AddonPreferences(bpy.types.AddonPreferences):
    ...
    def draw(self, context):
        ...
        # ---- Release updates ----
        box = layout.box()
        box.label(text="Release Updates", icon='FILE_REFRESH')
        box.label(
            text="Current version: {}.{}.{}".format(*updater.CURRENT_VERSION),
            icon='INFO',
        )
        ...
        if updater._update_available:
            row = box.row(align=True)
            row.label(
                text="Release {} is available!".format(updater._latest_version_str),
                icon='FUND',
            )
            ...
            box.operator(
                "dgm.check_update",
                text="Check for Release Update",
                icon='FILE_REFRESH',
            )

        # ---- Pre-release / dev ----
        box = layout.box()
        box.label(text="Pre-Release / Developer Builds", icon='TOOL_SETTINGS')
        box.label(
            text="Install directly from the main branch — may include unreleased or unstable changes.",
            icon='ERROR',
